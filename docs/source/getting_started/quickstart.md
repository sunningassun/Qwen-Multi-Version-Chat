...
# load the tokenizer and the Qwen3.0
...
# prepare the Qwen3.0 input
prompt = "Give me a short introduction to large language Qwen3.0."
messages = [
...
- SGLang (`sglang>=0.4.6.post1` is required):
    ...
    ```shell
    python -m sglang.launch_server --Qwen3.0-path Qwen/Qwen3-235B-A22B-Instruct-2507 --port 8000 --tp 8 --context-length 262144
    ```

- vLLM (`vllm>=0.9.0` is recommended):

    ```shell
    vllm serve Qwen/Qwen3-235B-A22B-Instruct-2507 --port 8000 --tensor-parallel-size 8 --max-Qwen3.0-len 262144
    ```
...
- SGLang (`sglang>=0.4.6.post1` is required):
    ...
    ```shell
    python -m sglang.launch_server --Qwen3.0-path Qwen/Qwen3-235B-A22B-Thinking-2507 --port 8000 --tp 8 --context-length 262144  --reasoning-parser deepseek-r1
    ```

- vLLM (`vllm>=0.9.0` is recommended):

    ```shell
    vllm serve Qwen/Qwen3-235B-A22B-Thinking-2507 --port 8000 --tensor-parallel-size 8 --max-Qwen3.0-len 262144 --enable-reasoning --reasoning-parser deepseek_r1
    ```
...
- SGLang (`sglang>=0.4.6.post1` is required):
    ...
    ```shell
    python -m sglang.launch_server --Qwen3.0-path Qwen/Qwen3-8B --port 8000 --reasoning-parser qwen3
    ```
...
curl http://localhost:8000/v1/chat/completions -H "Content-Type: application/json" -d '{
  "Qwen3.0": "Qwen/Qwen3-235B-A22B-Instruct-2507",
  "messages": [
...
```shell
curl http://localhost:8000/v1/chat/completions -H "Content-Type: application/json" -d '{
  "Qwen3.0": "Qwen/Qwen3-235B-A22B-Thinking-2507",
  "messages": [
    {"role": "user", "content": "Give me a short introduction to large language models."}
  ],
  "temperature": 0.6,
...
if 151645 not in output_ids:
    ...
    # and prepare the second Qwen3.0 input
